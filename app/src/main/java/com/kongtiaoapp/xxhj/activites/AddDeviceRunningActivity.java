 ...
 * 添加运行中中设备信息页面
 */
public class AddDeviceRunningActivity extends BaseActivity<AddDeviceRunningPresenter, AddDeviceRunningView> implements AddDeviceRunningView, MyScrollView.OnScrollListener, AbsListView.OnScrollListener {
    @ViewInject(R.id.listview)
    private DragListView mListview;
    @ViewInject(R.id.addDeviceInfo_tv_save)
    private TextView save;
    @ViewInject(R.id.txt_setting_draw)
    private TextView txt_setting_draw;//设置拖动
    @ViewInject(R.id.addDeviceInfo_tv_save)
    private TextView addDeviceInfo_tv_save;
    @ViewInject(R.id.line_add_device_info)
    private LinearLayout line_add_device_info;
    @ViewInject(R.id.tv_title)
    private TextView tvTitle;
    @ViewInject(R.id.txt_setting_font_small)//小号字体
            TextView txt_setting_font_small;
    @ViewInject(R.id.txt_setting_font_mid)//中号字体
            TextView txt_setting_font_mid;
    @ViewInject(R.id.txt_setting_font_big)//大号字体
    ...
    @Event(value = {R.id.iv_back, R.id.addDeviceInfo_tv_save, R.id.txt_setting_draw, R.id.txt_setting_font_small, R.id.txt_setting_font_mid, R.id.txt_setting_font_big})
    ...
    private void save() {
            ...
            if (fieldType.equals("float")) {
                ...
                if (content != null && !content.equals("")) {
                    if (Pattern.compile(PatternUtils.getDigital()).matcher(content).matches() || content.equals("")) {
                        ...
                        ToastUtils.showToast(AddDeviceRunningActivity.this, "请正确录入参数");
                        return;
                    }
        ...
        if (map.size() == 1) {//之前已经有mup.pu("deviceId");
            ToastUtils.showToast(this, getString(R.string.null_save));
            return;
        }
    ...
    @Override
    protected AddDeviceRunningPresenter getPresenter() {
    ...
    public void onScroll(int scrollY) {
        if (scrollY > 0) {
            ((InputMethodManager) getSystemService(INPUT_METHOD_SERVICE)).hideSoftInputFromWindow(AddDeviceRunningActivity.this.getCurrentFocus().getWindowToken(), InputMethodManager.HIDE_NOT_ALWAYS);
        }
    }
    ...
    public void setList(Object data) {
        ...
        adapter = new DeviceRunningParamAdapter(mList, AddDeviceRunningActivity.this, isAuto);
    ...
    public void saveDrag_CommitData_View(Object data) {
        ToastUtils.showToast(AddDeviceRunningActivity.this, getResources().getString(R.string.drag_success));
    }
