...
import com.kongtiaoapp.xxhj.mvp.presenter.SelectionDetailPresenter;
import com.kongtiaoapp.xxhj.mvp.view.SelectionDetailView;
 ...
 * 说明:精选详情页
 */
public class SelectionDetailActivity extends BaseActivity<SelectionDetailPresenter, SelectionDetailView> implements SelectionDetailView, AdapterView.OnItemClickListener {
...
//                    ShowToastDialog("是否删除该评论?", 1);
    ...
    @Override
    protected SelectionDetailPresenter getPresenter() {
        return new SelectionDetailPresenter();
    }
...
//        settings.setLayoutAlgorithm(WebSettings.LayoutAlgorithm.SINGLE_COLUMN);
    ...
    @Override
    public void comment_View(Object response) {
    ...
    public void showMeunDialog() {
        ...
        cancel.setOnClickListener(v -> dialog.cancelDialog());
        dialog.showDialog();
    }
    ...
    public void setList(Object response) {
        ...
        webView.loadData(data.getResobj().getSelectedInfo(), "text/html; charset=UTF-8", null);
        if (item.getComments() != null && item.getComments().size() > 0) {
            if (mList != null) {
                mList.clear();
                mList.addAll(item.getComments());
