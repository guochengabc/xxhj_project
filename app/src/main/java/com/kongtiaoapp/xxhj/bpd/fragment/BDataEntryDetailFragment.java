...
import com.kongtiaoapp.xxhj.mvp.presenter.BDataEntryDetailP;
...
/**
 * Created by G_XXHJ on 2018/9/14.  变配电数据录入详情
 */

public class BDataEntryDetailFragment extends BaseFragment<BDataEntryDetailP, BDataEntryDetailV> implements BDataEntryDetailV,
    ...
    private static BDataEntryDetailFragment instance = null;
    // 监听器在手机拖动停下的时候触发
    ...
    public synchronized static BDataEntryDetailFragment getInstance() {
        if (instance == null) {
            instance = new BDataEntryDetailFragment();
        }
        return instance;
    }
    ...
    private void save() {
        ...
        try {
            ...
            try {
                ...
                String recordFormResult = "{" + "\"" + "method" + "\"" + ":" + "\"" + HttpMethod.POSTPOWERPARAMDATA + "\"" + "," + "\"" + "reqobj" + "\"" + ":" + result.toString() + "}";
    ...
    @Override
    public BDataEntryDetailP getPresenter() {
        return new BDataEntryDetailP();
    }
    ...
    public void initData() {
        ...
        if (bundle != null) {
            BPD_DataEntryBean.ResobjBean.PowerBean bean = (BPD_DataEntryBean.ResobjBean.PowerBean) bundle.getSerializable("bpd");
            if (bean != null) {
                setData(bean);
            }
        }
    ...
    public void setData(BPD_DataEntryBean.ResobjBean.PowerBean bean) {
    ...
    private void setInitData(BPD_DataEntryBean.ResobjBean.PowerBean eCode) {
     ...
     /*   if (isAuto) {
        ...
        mListview.setDragEnabled(false);
        //非自动
        presenter.onResume(mActivity, eCode.getDeviceId());
    }
