 ...
 * 能源表单录入
 */
public class EnergyRecordActivity extends BaseGuestorActivity<EnergyRecordP, EnergyRecordV> implements EnergyRecordV, RecordStatus {
    ...
    @BindView(R.id.ctv_gasRecharge)
    ClickTextView ctv_gasRecharge;//燃气充值
    ...
    protected void initData() {
        ...
        if (intent != null) {
            ...
            if (recordType == null || recordType.trim().equals("") || recordType.trim().equals("normal")) {//正常录入
                recordType = "normal";
            } else {
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.iv_back:
                ...
                finish();
                break;
            case R.id.ctv_gasRecharge://燃气充值
                startActivity(new Intent(this, EnergyRecordActivity.class).putExtra("device", (Serializable) deviceNameE_codeBean).putExtra("recordType", "gasCharge"));
                finish();
                break;
    ...
    private void commitData() {
        ...
        if (adapter.getRecordStatus().equals("1")) {//如果事异常状态
            if (selImageList.isEmpty()) {
                ToastUtils.showToast(EnergyRecordActivity.this, "请上传异常照片");
                return;
            }
        }

        presenter.commitData(EnergyRecordActivity.this, list, selImageList, sensorId, adapter.getRecordStatus());
