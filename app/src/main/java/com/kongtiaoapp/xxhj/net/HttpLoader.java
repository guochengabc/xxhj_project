...
public class HttpLoader {
     ...
     */
    private static final HashMap<Integer, Request> mInFlightRequests = new HashMap<Integer, Request>();
    /**
     * 消息队列，全局使用一个
     */
 ...
 /*   public static ImageLoader getImageLoader() {
    ...
    private static void request(int method, String url, Map<String, String> params, Class<? extends RBResponse> clazz, final int requestCode, final ResponseListener listener, boolean isCache) {
        if (mInFlightRequests.get(requestCode) == null) {
            GsonRequest request = makeRBRequest(method, url + buildGetParam(params) + "&token=" + App.sp.getToken(), null, clazz, requestCode, listener, isCache);
    ...
    private static GsonRequest makeRBRequest(int method, String url, Map<String, String> params, Class<? extends RBResponse> clazz, int requestCode, ResponseListener listener, boolean isCache) {
        ...
        request.setRetryPolicy(( new DefaultRetryPolicy(80000,
              1,//qing
                2f)));//设置超时时间，重试次数，重试因子（1,1*2,2*2,4*2）等
    ...
    public interface ResponseListener {
        ...
        void onGetResponseFailed(int requestCode, int code);
    }
    ...
    private static class RequestListener implements Response.ErrorListener, Response.Listener<RBResponse> {
        ...
        public void onErrorResponse(VolleyError error) {
            ...
            ToastUtils.showToast(App.application,App.application.getString(R.string.no_net));
            //请求失败，解析本地缓存
            ALog.w("服务器请求失败，使用本地缓存数据！"+error.getMessage());
            getCacheResponse(request);
        }


        @Override
        public void onResponse(RBResponse code) {
            mInFlightRequests.remove(requestCode);//请求成功，从正在飞的集合中删除该请求
            if (code != null) {
                if (code.getCode() == 40001) {
                    if (!TextUtils.isEmpty(code.getErrormsg())) {
                        ToastUtils.showToast(App.application,code.getErrormsg());
                    } else {
                        ToastUtils.showToast(App.application,App.application.getString(R.string.param_fail));
                    }
                    if (listener != null) {
                        listener.onGetResponseFailed(requestCode, code.getCode());
                    }
                    return;
                } else if (code.getCode() == 40002) {
                    if (!TextUtils.isEmpty(code.getErrormsg())) {
                        ToastUtils.showToast(App.application,code.getErrormsg());
                    } else {
                        ToastUtils.showToast(App.application,App.application.getString(R.string.error_net));
                    }
                    if (listener != null) {
                        listener.onGetResponseFailed(requestCode, code.getCode());
                    }
                    return;
                } else if (code.getCode() == 40003) {
                    if (!TextUtils.isEmpty(code.getErrormsg())) {
                        ToastUtils.showToast(App.application,App.application.getString(R.string.error_net));
                    } else {
                        ToastUtils.showToast(App.application,App.application.getString(R.string.centeridentify_error));
                    }
                    if (listener != null) {
                        listener.onGetResponseFailed(requestCode, code.getCode());
                    }
                    return;
                } else if (code.getCode() == 40004) {
                    if (!TextUtils.isEmpty(code.getErrormsg())) {
                        ToastUtils.showToast(App.application, code.getErrormsg());
                    } else {
                        ToastUtils.showToast(App.application,App.application.getString(R.string.execute_error));
                    }
                    if (listener != null) {
                        listener.onGetResponseFailed(requestCode, code.getCode());
                    }
                    return;
                } else if (code.getCode() == 40006) {
                    if (!TextUtils.isEmpty(code.getErrormsg())) {
                        App.sp.setLoginState(false);
                        App.sp.resetSp();
                        System.gc();
                        AllActivityManager.getInstance().finishAllActivity();
                        Intent intent = new Intent(App.application, LoginActivity.class);
                        intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                        intent.putExtra("quit_login","quit_login");
                        App.application.startActivity(intent);
                    } else {
                        ToastUtils.showToast(App.application,App.application.getString(R.string.getToken_error));
                    }

                    return;
                } else if (code.getCode() == 40007) {
                    if (!TextUtils.isEmpty(code.getErrormsg())) {
                        ToastUtils.showToast(App.application,code.getErrormsg());
                    } else {
                        ToastUtils.showToast(App.application,App.application.getString(R.string.user_noExist));
                    }
                    if (listener != null) {
                        listener.onGetResponseFailed(requestCode, code.getCode());
                    }
                    return;
                } else if (code.getCode() == 40008) {
                    if (!TextUtils.isEmpty(code.getErrormsg())) {
                        ToastUtils.showToast(App.application, code.getErrormsg());
                    } else {
                        ToastUtils.showToast(App.application, App.application.getString(R.string.param_null));
                    }
                    if (listener != null) {
                        listener.onGetResponseFailed(requestCode, code.getCode());
                    }
                    return;
                } else if (code.getCode() == 40009) {
                    if (!TextUtils.isEmpty(code.getErrormsg())) {
                        ToastUtils.showToast(App.application, code.getErrormsg());
                    } else {
                        ToastUtils.showToast(App.application, App.application.getString(R.string.user_exist));
                    }
                    if (listener != null) {
                        listener.onGetResponseFailed(requestCode, code.getCode());
                    }
                ...
                if (listener != null) {
                    listener.onGetResponseSuccess(requestCode, code);
                }
            }
        }
