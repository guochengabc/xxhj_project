 ...
 */
public class EnergyStoreListAdapter extends BaseAdapter {
    private Context mContext;
    private List mList;


    public EnergyStoreListAdapter(Context context, List list) {
...
//            convertView =  inflater.inflate(R.layout.whatever, parent, false);
        ...
        GetProductList.Product item = (GetProductList.Product) mList.get(position);
        Picasso.with(mContext).load(ConstantValue.URL +item.getPhotoUrl()).placeholder(R.mipmap.default_head).into(holder.ivPicture);
        holder.tvContent.setText(item.getName() + item.getPrice() + item.getDescription());
