...
public class MyUpdateService extends Service {
    ...
    private void initOurApk() {
        Uri uri = Uri.fromFile(FileUtil.updateFile);
        Intent intent = new Intent(Intent.ACTION_VIEW);
        intent.setDataAndType(uri,"application/vnd.android.package-archive");
        pendingIntent = PendingIntent.getActivity(MyUpdateService.this, 0, intent, 0);
        notification.flags = Notification.FLAG_AUTO_CANCEL;
        builder1.setContentIntent(pendingIntent);
    ...
    private void initHGApk() {
        Uri uri = Uri.fromFile(FileUtil.updateFile_work);
        Intent intent = new Intent(Intent.ACTION_VIEW);
        intent.setDataAndType(uri,"application/vnd.android.package-archive");
        pendingIntent = PendingIntent.getActivity(MyUpdateService.this, 0, intent, 0);
        notification.flags = Notification.FLAG_AUTO_CANCEL;
    ...
    public void createNotification() {
        ...
        contentView.setProgressBar(R.id.pro_notificationProgress, 100, 0, false);
        builder1 = new Notification.Builder(this);
        builder1.setSmallIcon(R.mipmap.ic_launcher); //设置图标
        builder1.setTicker("显示第二个通知");
        builder1.setWhen(System.currentTimeMillis()); //发送时间
        builder1.setDefaults(Notification.FLAG_NO_CLEAR); //设置默认的提示音，振动方式，灯光
    ...
    public long downloadUpdateFile(String down_url, String file)throws Exception {
        ...
        int updateCount = 0;// 已经上传的文件大小所占的百分比
        ...
        while ((readsize = inputStream.read(buffer)) != -1) {
            ...
            if (updateCount == 0 || ((downloadCount * 100) / totalSize - down_step) >= updateCount ) {
                ...
                App.sp.setCheckCount(updateCount+"%");
                contentView.setProgressBar(R.id.pro_notificationProgress, 100,updateCount, false);
                builder1.setContent(contentView);
                notificationManager.notify(R.layout.notification_item, notification);
            }
        }
    ...
    public void onDestroy() {
        super.onDestroy();
        if (downLoadThread.isAlive() && downLoadThread!=null){
