...
/**
 * Created by 桂飞 on 2016/11/7.     柱状图
 */

public class BarChart extends AbstractOneChart {
       ...
       /* setChartSettings(context, renderer, "主标题", "x轴标题", "百分比%", 0, xMax, 0, yMax,
        ...
        if (yMax < 10){
            if (minY >= 0) {
                ...
                if (isMonth) {
                    setChartSettings(context, renderer, "", xtitile, "", xMin, xMax, minY - 2, 10,
                            Color.LTGRAY, Color.LTGRAY);
                } else {
                    setChartSettings(context, renderer, "", xtitile, "", xMin, xMax, minY - 2, 10,
                            Color.LTGRAY, Color.LTGRAY);
        ...
        //   renderer.setBarWidth(5);   //BarChart.Type.DEFAULT  左右排列   BarChart.Type.STACKED重叠排列
        GraphicalView timeChartView = ChartFactory.getBarChartView(context, dataset, renderer, org.achartengine.chart.BarChart.Type.DEFAULT);
