 ...
 */
public class BarChartNoRealTime_String extends AbstractOneChart {
    ...
    public GraphicalView execute(Context context, String[] titles, List<String> x, List<double[]> yvalues, String xtitile, double xMax, double yMax, double minY, boolean isMonth, long nowTime) {
        ...
        double[] dy = new double[4 * x.size() * yvalues.size()];
        int xSizeSpace = 4;
        int xPaintSpace = 4;

        for (int j = 0; j < x.size(); j++) {
            renderer.addXTextLabel(xSizeSpace * j + 2, x.get(j));
            for (int i = 0; i < yvalues.size(); i++) {
                dy[xPaintSpace * j + 1] = yvalues.get(i)[j];
            }
        }
       ...
       /* setChartSettings(context, renderer, "主标题", "x轴标题", "百分比%", 0, xMax, 0, yMax,
        ...
        if (yMax < 0.5) {
            if (minY >= 0) {
                setChartSettings(context, renderer, "", xtitile, "", xMin, xMax, 0, 0.5,
                        Color.LTGRAY, Color.LTGRAY);
            } else {
                setChartSettings(context, renderer, "", xtitile, "", xMin, xMax, minY, 0.5,
                        Color.LTGRAY, Color.LTGRAY);
            }
        } else if (yMax < 1) {
            if (minY >= 0) {
        ...
        } else if (yMax >= 1 && yMax < 3) {
            if (minY >= 0) {
                setChartSettings(context, renderer, "", xtitile, "", xMin, xMax, 0, 3,
                        Color.LTGRAY, Color.LTGRAY);
            } else {
                setChartSettings(context, renderer, "", xtitile, "", xMin, xMax, minY, 3,
                        Color.LTGRAY, Color.LTGRAY);
            }
        } else if (yMax >= 3 && yMax < 8) {
            if (minY >= 0) {
        ...
        if (isMonth) {
            renderer.setXLabels(0);//0为不显示x轴的值
        } else {
