...
public abstract class AbstractDemoChart implements IDemoChart {
     ...
     */
    protected CategorySeries buildCategoryDataset(String title, double[] values, String[] name) {
        CategorySeries series = new CategorySeries(title);
        int k = 0;
        for (int i = 0; i < values.length; i++) {
            series.add(name[i], values[i]);//设置图例和名字
        }
    ...
    protected DefaultRenderer buildCategoryRenderer(int[] colors) {
        ...
        renderer.setLabelsColor(R.color.a333333);
        renderer.setMargins(new int[]{10, 20, 10, 0});
