package com.kongtiaoapp.xxhj.ui.timeselector.lib;
...
import com.kongtiaoapp.xxhj.ui.timeselector.adapter.WheelAdapter;
import com.kongtiaoapp.xxhj.ui.timeselector.listener.OnItemSelectedListener;
...
public class WheelView extends View {
    ...
    int textSize;//选项的文字大�?
    boolean customTextSize;//自定义文字大小，为true则用于使setTextSize函数无效，只能�?�过xml修改
    int maxTextWidth;
    int maxTextHeight;
    ...
    private int drawCenterContentStart = 0;//中间选中文字�?始绘制位�?
    private int drawOutContentStart = 0;//非中间文字开始绘制位�?
    private static final float SCALECONTENT = 0.8F;//非中间文字则用此控制高度，压扁形�?3d错觉
    private static final float CENTERCONTENTOFFSET = 6;//中间文字文字居中�?要此偏移�?
    ...
    public WheelView(Context context, AttributeSet attrs) {
        ...
        if(attrs != null) {
            ...
            dividerColor = a.getColor(R.styleable.wheelview_dividerColors,dividerColor);
    ...
    private void remeasure() {
        ...
        //�?大Text的高度乘间距倍数得到 可见文字实际的�?�高度，半圆的周�?
        halfCircumference = (int) (itemHeight * (itemsVisible - 1)) ;
        //整个圆的周长除以PI得到直径，这个直径用作控件的总高�?
    ...
    void smoothScroll(ACTION action) {
        ...
        //停止的时候，位置有偏移，不是全部都能正确停止到中间位置的，这里把文字位置挪回中间�?
    ...
    public final void setCurrentItem(int currentItem) {
        ...
        totalScrollY = 0;//回归顶部，不然重设setCurrentItem的话位置会偏移的，就会显示出不对位置的数�?
        invalidate();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //滚动的Y值高度除去每行Item的高度，得到滚动了多少个item，即change�?
        change = (int) (totalScrollY / itemHeight);
        try {
            //滚动中实际的预�?�中的item(即经过了中间位置的item) �? 滑动前的位置 �? 滑动相对位置
        ...
        if (!isLoop) {//不循环的情况
            ...
            if (preCurrentIndex < 0) {//举个例子：如果�?�数�?5，preCurrentIndex �? �?1，那么preCurrentIndex按循环来说，其实�?0的上面，也就�?4的位�?
                preCurrentIndex = adapter.getItemsCount() + preCurrentIndex;
            }
            if (preCurrentIndex > adapter.getItemsCount() - 1) {//同理上面,自己脑补�?�?
                preCurrentIndex = preCurrentIndex - adapter.getItemsCount();
            }
        }

        //跟滚动流畅度有关，�?�滑动距离与每个item高度取余，即并不是一格格的滚动，每个item不一定滚到对应Rect里的，这个item对应格子的偏移�??
        int itemHeightOffset = (int) (totalScrollY % itemHeight);
        // 设置数组中每个元素的�?
        int counter = 0;
        while (counter < itemsVisible) {
            int index = preCurrentIndex - (itemsVisible / 2 - counter);//索引值，即当前在控件中间的item看作数据源的中间，计算出相对源数据源的index�?

            //判断是否循环，如果是循环数据源也使用相对循环的position获取对应的item值，如果不是循环则超出数据源范围使用""空白字符串填充，在界面上形成空白无数据的item�?
            if (isLoop) {
        ...
        while (counter < itemsVisible) {
            ...
            if (angle >= 90F || angle <= -90F) {
                ...
                //根据Math.sin(radian)来更改canvas坐标系原点，然后缩放画布，使得文字高度进行缩放，形成弧形3d视觉�?
    ...
    /**
     * 根据传进来的对象反射出getPickerViewText()方法，来获取�?要显示的�?
     * @param item
     * @return
     */
