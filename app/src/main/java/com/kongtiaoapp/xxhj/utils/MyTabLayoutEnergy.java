...
public class MyTabLayoutEnergy  extends TabLayout implements TabLayout.OnTabSelectedListener,ViewPager.OnPageChangeListener{

    private int tabPosition = 0;
    private int length;
    private DefinationViewpagerEnergy dv_recorder;
    ...
    public MyTabLayoutEnergy(Context context) {
        super(context);
    ...
    public MyTabLayoutEnergy(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.setTabMode(MODE_SCROLLABLE);
        //  setTabNumber();
    ...
    public MyTabLayoutEnergy(Context context, AttributeSet attrs, int defStyleAttr) {
     ...
     * 设置tablayout相关参数
     */
    public void addTablayoutSetting(Context mContext, FragmentManager fragmentManager, List<Fragment> list_fragment, String[] title, DefinationViewpagerEnergy dv_recorder) {
        ...
        dv_recorder.addOnPageChangeListener(this);
        ...
        this.post(new Runnable() {
            @Override
            public void run() {
                setIndicator(MyTabLayoutEnergy.this, 50, 50);
            }
        });
    }

    public void addTablayoutChanges(Context mContext, List<Fragment> list_fragment, String[] title, DefinationViewpagerEnergy dv_recorder, TextView tv_weiBao) {
    ...
    public void addTablayoutChanges(Context mContext, List<Fragment> list_fragment, String[] title, DefinationViewpagerEnergy dv_recorder) {
        this.length = title.length;
        if (this.length <= 3) {
            this.setTabMode(MODE_FIXED);
        } else {
    ...
    public void onPageSelected(int position) {
        try {
            View view = dv_recorder.getChildAt(position);
            int height = view.getMeasuredHeight();
            LayoutParams layoutParams = (LayoutParams) dv_recorder.getLayoutParams();
            layoutParams.height = height;
            dv_recorder.setLayoutParams(layoutParams);
        } catch (Exception e) {
            e.printStackTrace();
        }
